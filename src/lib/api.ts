...
export function createRoom(roomIndex: number): string | null {
    ...
    if (check(res, { 'room created': (r: RefinedResponse<ResponseType>) => r.status === 200 || r.status === 201 })) {
...
export function createLesson(roomId: string): string | null {
    const res = http.post(
        ...
        JSON.stringify({}),
        { headers: getHeaders(CONFIG.TEACHER_TOKEN), tags: { name: 'create_lesson', room: roomId } }
    );

    if (check(res, { 'lesson created': (r: RefinedResponse<ResponseType>) => r.status === 200 || r.status === 201 })) {
        httpSuccess.createLesson.add(1);
        roomLessonCreated.add(1);
        const data = res.json() as { data?: { lesson_id: string }; lesson_id?: string; id?: string };
...
export function getLesson(roomId: string): string | null {
    const res = http.post(
        `${CONFIG.API_URL}/api/v3/rooms/${roomId}/lessons`,
        JSON.stringify({}),
        { headers: getHeaders(CONFIG.TEACHER_TOKEN), tags: { name: 'get_lesson', room: roomId } }
    );
...
export function startLesson(lessonId: string): boolean {
    const res = http.put(
        ...
        { headers: getHeaders(CONFIG.TEACHER_TOKEN), tags: { name: 'start_lesson' } }
    );
    if (check(res, { 'lesson started': (r: RefinedResponse<ResponseType>) => r.status === 200 })) {
...
export function endLesson(lessonId: string): boolean {
    ...
    if (check(res, { 'lesson ended': (r: RefinedResponse<ResponseType>) => r.status === 200 })) {
...
export function chooseSeat(
    ...
    const res = http.post(
        ...
        { headers: getHeaders(), tags: { name: 'choose_seat', room: roomId }, timeout: '30s' }
    );

    seatTime.add(Date.now() - startTime);

    if (check(res, { 'seat chosen': (r: RefinedResponse<ResponseType>) => r.status === 200 || r.status === 201 })) {
...
export function createQuizzes(lessonId: string): string | null {
        ...
        quizzes: [
            {
                ...
                seq: 1,
                option_list: [
                    { option_id: 1, content: 'True', is_ai_answer: true, is_answer: true },
    ...
    const res = http.post(
        ...
        JSON.stringify(payload),
        { headers: getHeaders(CONFIG.TEACHER_TOKEN), tags: { name: 'create_quizzes' }, timeout: '30s' }
    );

    if (res.status === 200 || res.status === 201) {
        httpSuccess.createQuizzes.add(1);
        const data = res.json() as { data?: { batch_quizzes_id: string }; batch_quizzes_id?: string; id?: string };
        return data.data?.batch_quizzes_id || data.batch_quizzes_id || data.id || null;
    }
...
export function fetchQuiz(lessonId: string, studentId: string): QuizResponse | null {
    const res = http.get(
...
export function submitAnswers(batchId: string, studentId: string, quizData: QuizResponse): boolean {
    const startTime = Date.now();
    ...
    if (check(res, { 'answers submitted': (r: RefinedResponse<ResponseType>) => r.status === 200 || r.status === 201 })) {
...
export function finishQuiz(lessonId: string, batchId: string): boolean {
    ...
    if (check(res, { 'quiz finished': (r: RefinedResponse<ResponseType>) => r.status === 200 })) {
...
export function closeQuiz(lessonId: string, batchId: string): boolean {
    ...
    if (check(res, { 'quiz closed': (r: RefinedResponse<ResponseType>) => r.status === 200 })) {
...
export function discloseQuiz(lessonId: string, batchId: string): boolean {
    const res = http.put(
        ...
        null,
        { headers: getHeaders(CONFIG.TEACHER_TOKEN), tags: { name: 'disclose_quiz' } }
    );
    if (check(res, { 'quiz disclosed': (r: RefinedResponse<ResponseType>) => r.status === 200 })) {
...
export function addStudentPoints(lessonId: string, students: StudentPoints[]): boolean {
    const res = http.put(
        `${CONFIG.API_URL}/api/v3/lessons/${lessonId}/batch_points`,
        JSON.stringify({ students }),
        { headers: getHeaders(CONFIG.TEACHER_TOKEN), tags: { name: 'add_points' } }
    );
    if (check(res, { 'points added': (r: RefinedResponse<ResponseType>) => r.status === 200 })) {
