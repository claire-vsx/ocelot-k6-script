...
// === HTTP 錯誤計數 ===
type HttpErrorCode = 400 | 401 | 403 | 404 | 409 | 422 | 429 | 500 | 502 | 503 | 504 | 'timeout' | 'other';
...
// === choose_seat 錯誤 ===
type SeatErrorCode = 400 | 401 | 403 | 404 | 409 | 422 | 429 | 500 | 'timeout' | 'other';
...
export function recordHttpError(status: number, endpoint?: string): void {
    const key: HttpErrorCode = status === 0
        ? 'timeout'
        : (httpErrors[status as keyof typeof httpErrors] ? status as HttpErrorCode : 'other');
    httpErrors[key].add(1);

    if (endpoint === 'choose_seat') {
        const seatKey: SeatErrorCode = status === 0
            ? 'timeout'
            : (seatErrors[status as keyof typeof seatErrors] ? status as SeatErrorCode : 'other');
